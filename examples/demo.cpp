...
int main() {
    ...
    // 2. RGB uint8 image. 2x4 color chart makes channel/order mistakes obvious.
    {
        ...
        for (size_t r = 0; r < rows; ++r) {
            for (size_t c = 0; c < cols; ++c) {
                const size_t tile_r = r / (rows / 2);
                const size_t tile_c = c / (cols / 4);
        ...
        for (size_t r = 0; r < n; ++r)
            for (size_t c = 0; c < n; ++c)
                gauss[r * n + c] = pi * std::exp(
                    -0.5 * ((static_cast<double>(c) - center) * (static_cast<double>(c) - center)
                          + (static_cast<double>(r) - center) * (static_cast<double>(r) - center))
                    / (sigma * sigma));
    ...
    // 5. 8x8 integer ramp, center-cropped to a 6x4 chip around (4,4)
    {
        ...
        for (size_t i = 0; i < 64; ++i)
            mat[i] = static_cast<std::int32_t>(i);
    ...
    //    (the default) downsamples it with bilinear interpolation.
    {
        ...
        for (size_t r = 0; r < n; ++r) {
            double y = -3.0 + 6.0 * static_cast<double>(r) / static_cast<double>(n - 1);
            for (size_t c = 0; c < n; ++c) {
                double x = -3.0 + 6.0 * static_cast<double>(c) / static_cast<double>(n - 1);
                peaks[r * n + c] =
                    3.0 * (1 - x) * (1 - x)
                        * std::exp(-x * x - (y + 1) * (y + 1))
